...
############~~~~~~~~~~~************************~~~~~~~~~~############
#####################################################################
# Created by Ismael V. Brack - Nov/2019

simul_Nmix <- function(S=200,J=8,obs2=.5,lambda=.3,phi=.4,p=.8,returnData=F){
  #* S: sites
  #* J: visits
  ...
  if(obs2==0){
    ...
    m0 <- pcount(~1 ~1,data=umf,mixture="P",starts=c(qlogis(phi*p),log(lambda)),K=200)
    
    # saving results...
    resu <- data.frame(real=c(lambda,(phi*p)),
                       beta=coef(m0),
                       beta.se=SE(m0),
  ...
  if(obs2>0 & obs2<1){
    ...
    # fitting models... lambda, phi, p
    m0 <- gmultmix(~1,~1,~1,data=umf,mixture="P",starts=c(log(lambda),qlogis(phi),qlogis(p)),K=200)
  ...
  if(returnData==TRUE){
    return(list(result=resu,data=Y))
  }
  else{return(resu)}
